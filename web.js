...
.then((data) => {
    ...
    data.map((value) =>{
        data2 = `
        <ul>
        <li class="nav-links" id="mammals">${value.name1}</li>
        <li class="nav-links" id="rebtiles">${value.name2}</li>
        <li class="nav-links" id="birds">${value.name3}</li>
        <li class="nav-links" id="contact">${value.name4}</li>
        <li class="nav-links" id="last-li">${value.name5}</li>
        </ul>
        `
        document.querySelector('.middle-nav').innerHTML = data2
    })

    const mam = document.getElementById('mammals')
    const reb = document.getElementById('rebtiles')
    const bird = document.getElementById('birds')
    const cont = document.getElementById('contact')
    const me = document.getElementById('last-li')

    mam.addEventListener('click', ()=>{
        let el = document.getElementById('mammals-title')
        el.scrollIntoView({behavior: 'smooth', block: 'start'})
    })
    reb.addEventListener('click', ()=>{
        let el = document.getElementById('rebtiles-title')
        el.scrollIntoView()
    })
    bird.addEventListener('click', ()=>{
        let el = document.getElementById('birds-title')
        el.scrollIntoView()
    })
